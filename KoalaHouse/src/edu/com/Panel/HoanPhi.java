...
import edu.com.Dialog.ThemSuaHoanPhi;
import edu.com.XuLy;
 ...
 * @author Venus
 */
public class HoanPhi extends javax.swing.JPanel {
    
    private boolean isadmin=true;
    private DefaultTableModel model;
    ArrayList<Integer> id_hoanphi;
    public JTabbedPane center;
    /**
     * Creates new form HoanPhi
     */
    public HoanPhi() {
        initComponents();
       // System.out.println("da den doan nay");
        new DataBase.SQLHoanPhi().BandDanhSachHoanPhi(jTable4);
        id_hoanphi = new ArrayList<Integer>();
        try{
        if (!jTable4.getValueAt(0, 0).toString().equals("")) {
            XuLy.setID(id_hoanphi, jTable4, 0);
            resize(jTable4);
        }
        }
        catch(Exception ex)
    ...
    private void initComponents() {
        ...
        jTable4.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Học Sinh", "Lớp", "Điện Thoại Cha/Mẹ", "Số Tiền Hoàn", "Ngày Bắt Đầu", "Ngày Kết Thúc", "Ghi Chú", "Tình Trạng", "Đánh Dấu"
            }
        ) {
    ...
    private void jTable4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable4MouseClicked
        ...
        if (evt.getClickCount() == 2) {
            ...
            id = new DataBase.SQLHoanPhi().getIdStudent(tenHocSinh, tenlop);
    ...
    private void themMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_themMouseClicked
        ...
        if(isadmin)
        {
            ...
            ThemSuaHoanPhi  hoanphi= new ThemSuaHoanPhi(null, true);
            hoanphi.setLocation(Toolkit.getDefaultToolkit().getScreenSize().width/2-200,Toolkit.getDefaultToolkit().getScreenSize().height/2-200);

            hoanphi.setThemSuaLop(true);
            hoanphi.setVisible(true);
            new DataBase.SQLHoanPhi().BandDanhSachHoanPhi(jTable4);
            id_hoanphi = new ArrayList<Integer>();
            try{
                if (!jTable4.getValueAt(0, 0).toString().equals("")) {
    ...
    private void suaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_suaMouseClicked
        ...
        if(isadmin)
        {
            ...
            if(jTable4.getValueAt(row, 8).equals("Chưa Hoàn Phí"))
            {
                Vector vec =  (Vector) model.getDataVector().elementAt(row);
                ThemSuaHoanPhi  hoanphi= new ThemSuaHoanPhi(null, true,vec);
                hoanphi.setidxebus(id_hoanphi.get(row));
                hoanphi.setThemSuaLop(false);
                hoanphi.setLocation(Toolkit.getDefaultToolkit().getScreenSize().width/2-200,Toolkit.getDefaultToolkit().getScreenSize().height/2-200);
    ...
    private void xoaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_xoaMouseClicked
        ...
        if(isadmin)
        {
            ...
            if(!flags)
            {
                ...
                boolean xoaorkhong;
                DataBase.SQLHoanPhi data= new DataBase.SQLHoanPhi();
                xoaorkhong=data.xoaHoanPhi(jTable4, 9,id_hoanphi);
    ...
    private void textfield_timkiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_textfield_timkiemActionPerformed
        // TODO add your handling code here:
        new DataBase.SQLHoanPhi().BandDanhSachHoanPhi_TimKiem(jTable4,textfield_timkiem.getText());
        id_hoanphi = new ArrayList<Integer>();
        try{
            if (!jTable4.getValueAt(0, 0).toString().equals("")) {
                XuLy.setID(id_hoanphi, jTable4, 0);
